...
lazy val frontend = (project in file("frontend"))
  ...
  .settings(
    jsTool := JSToolConfig(
      installPackagesCommand = ScalaJsCli.Cmd("npm install --prefix frontend", "added\\s\\d+\\spackage".r),
      dev = DevConfig(
        command = ScalaJsCli.Cmd("npm run dev --prefix frontend", "VITE.*ready".r),
        ...
        successMessage = s"""|Development environment ready!\n Web app now available on http://localhost:9876
                             |Using backend url: ${Environtments.backendBaseUrl}
                             |Press Ctrl+C to stop""".stripMargin
      ),
      build = BuildConfig(
        command = ScalaJsCli.Cmd("npm run --prefix frontend build", "built in d+".r),
        startupMessage = "Bundling web app",
        successMessage = "Done"
      )
    )
  )
